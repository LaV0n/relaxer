...
export const CountryInput = () => {
   ...
   useEffect(() => {
      if (country) {
         setPersonName(
            Object.keys(countryList).filter(c =>
               country.split(',').includes(countryList[c as keyof typeof countryList])
            )
         )
      } else {
         setPersonName([])
   ...
   useEffect(() => {
      dispatch(
         setCountry(personName.map(c => countryList[c as keyof typeof countryList]).join(','))
      )
   }, [personName])
   ...
   return (
      <div>
         <FormControl className={styles.formBlock}>
            <InputLabel id="multiple-chip-label" sx={{ color: 'rgba(255, 255, 255, 0.4)' }}>
               Country
            </InputLabel>
            <Select
               ...
               MenuProps={MenuProps}
            >
               {Object.keys(countryList).map(name => (
